...
whereas she treats the entire project as one unit.

Members of our team prefer to code in different languages. This post is written
from the perspective of a (primarily) R user on a multilingual team.
...
This project style, largely developed by [Scott
Weikart](https://hrdag.org/people/scott-weikart-phd/) and [Jeff
Klingner](https://hrdag.org/people/jeff-klingner-phd/), is not only
...
1. Reproducible: Given the original input data, anyone should be able to pick up
   the project and re-create the output (whether that is a written report, some
   charts, a new data set, or what have you).
2. Auditable: If anything in the output looks surprising or seems wrong, I
   should be able to trace that output back through every transformation and
   modeling choice, to see exactly how the original input data resulted in that
   output.
...
empathy is valuble to cultivate because, if for no other reason, the person
...
Whenever code reads in some data from disk, or writes anything out to disk, we
face a reproducibility-related challenge: 

1. Code that relies on a relative filename and works in an interactive session,
   ...
   your R session, and sometimes it will produce the correct output and
   sometimes it won't, which fails any reasonable definition of reproducibility.
   Or alternatively it might be set explicitly using `setwd`, which is not only
   inelegant, but also runs into problem (2).

2. Code that relies on an absolute filename, such as
   `read.csv("/Users/tshah/projects/myproject/import/input/mydata.csv")`,
   depends on where the project happens to be on my machine. If I share the
   project with anyone else, that code that works for me will result in a "No
   such file or directory" error for anyone else, again failing most definitions
   of reproducibility.
...
But that approach doesn't really solve our problem. First of all, it's specific
to R. Recall that different tasks, or even bits of code within tasks, may be
written in a number of different languages. Furthermore, given that our own
definition of reproducibility included being able to run the task via `make`, we
haven't met our standard for reproducibility. In fact, this solution *makes
RStudio a project dependency*. That is a high cost to pay, especially since, as
mentioned above, the person trying to execute the task may not even be an R
user.

What can we do instead? We can use the `here` package, [as Jenny Bryan
recommends doing
here](https://www.tidyverse.org/blog/2017/12/workflow-vs-script/). Or,
similarly, we can use `git rev-parse --show-toplevel`. For example, here is a
recent Makefile of mine. The goal of this task is to take some simulated "ground
truth" data generated in an earlier task, and sample from it multiple times --
mimicking the process of different groups on the ground collecting evidence of
human rights abuses:
...
So when to use `here::here()` and when to use `git rev-parse --show-toplevel`?
...
determine the project root, without needing git or Rstudio. For this example, my
thought process went like this: this is a git repository, so the added
flexibility is not necessary. Given that anyone running the code would have
first had to clone the git repository, requiring `git` as a project dependency
feels lighter weight than requiring the `here` R package. The solution you
choose will depend on your own context.
...
makes work unreplicatable and can be the source of serious bugs that are
...
task. Read in a text file, get all of the column types right, and then write the
output in a binary format that is fast to read/write. The next task can start by
reading in the binary file.

### An aside about data file formats

The choice of format when writing data to disk depends on a number of factors.
Here are a few that come up routinely for us, and when we reach for each of them.

- csv or csv.gz: For data that will be used outside of a project, we prefer text
  formats (pipe-delimited text files, to be precise) to binary file formats. The
  useful life of a project tends to be quite long, so that projects can easily
  outlive specific file formats. Plain delimited text files, on the other hand,
  are delightfully robust to technological change. If the data is taking up too
  much disk space (which can make it difficult to share across a network, among
  other things), you can write the data to a compressed file instead. For
  example, in R:

```{r, eval = FALSE}
output_file <- gzfile("myoutput.csv.gz")
write.table(mydata, file = output_file,
            sep = "|", na = "",
            row.names = FALSE)
```

The three main shortcomings of delimited text are that (1) it can be slow to
read and write (though speedy options such as `data.table::fread` and
`data.table::fwrite` make this less of an issue), (2) there is no built-in,
consistent way to deal with missing values -- anyone wanting to read in your
data has to be aware of the convention you use, and (3) it doesn't store
information about column types (character, numeric, date, etc.), so you need to
be sure you're reading things in correctly every time you import the data. If
this data is a final product, or will be reviewed a lot on its own, then for the
reasons stated above text is probably still the way to go. In some cases,
though, data is only written to disk as an intermediate step. In these cases,
you may opt for a binary format. Here are the two I end up using the most often
and when I reach for each one:

- rds or rds.gz: If I know that the only downstream tasks that will consume the
  data are also written in R, then R's RDS file format has a lot to offer:
  reading and writing is quick, missing values and variable types are preserved
  during the reading/writing process, and we have the flexibility to store
  structures that aren't data frames (lists, environments, functions, and so
  forth). On its own, the RDS format can take up more room than a plain
  delimited text file, but `base::saveRDS` (and `readr::write_rds`) includes an
  option to save to a compressed file.
- [feather](https://cran.r-project.org/web/packages/feather/index.html): feather
  is a nice binary format for cross-language collaborations. Feather files
  correctly handle missing values and retain information about column types, and
  are quick to read and write. The data takes more space on disk than a text
  file would, but can be read in a variety of languages including R, Python, and
  Julia.
...
As with any writing, good readable code will often require you to re-visit and
re-organize and ruthlessly edit. The goal is not a video recording of every
button you pressed to get some output, it is to describe in a human-readable way
how the input becomes the output, and to provide instructions for the machine to
produce that transformation. So after reviewing the above code with an editor's
eye, I might end up with:
